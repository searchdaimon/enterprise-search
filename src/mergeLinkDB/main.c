...
static inline int int_compare(unsigned int a, unsigned int b) {
	if (a < b)
		return -1;
	if (a > b)
		return 1;
 ...
 */
static inline void read_next_blocks(struct linkdb_block * block_a, struct linkdb_block * block_b, 
 ...
 */
static inline int find_by_from(struct linkdb_block * block_a, struct linkdb_block * block_b) {
 ...
 * to output.
 */
static inline int find_next_block(struct linkdb_block * block_a, struct linkdb_block * block_b) {
	int block_out;
...
static inline void write_out(FILE * output, struct linkdb_block * db_block) {
 ...
 */
static inline void dump_remaining(FILE * file_a, FILE * file_b, FILE * out) {
	FILE * in;
...
int main (int argc, char *argv[]) {
	...
	if ((linkfile_a = open_db_file(argv[1])) == NULL) {
		printf("dosent have a link db. Creating one from \"%s\"\n",argv[2]);
		sprintf(sysbuf,"cp %s %s",argv[2],argv[1]);
		printf("runing: \"%s\"\n",sysbuf);
		system(sysbuf);
		exit(1);
	}

	if ((linkfile_b = open_db_file(argv[2])) == NULL) {
		perror(argv[2]);
		exit(1);
	}
	...
	*/
	...
	sprintf(sysbuf,"mv %s %s",outfile,argv[1]);
	printf("runing: \"%s\"\n",sysbuf);
	system(sysbuf);
	/*
