...
%s.%i.%s",cashedir,QueryData.queryhtml,QueryData.start,QueryData.GeoIPcontry);
...
"Not all the search nodes responded to your query. Result quality may have been negatively effected.");
	}
...
derHeder[i].queryTime.crawlManager);
							printf("\t<getUserObjekt>%f</getUserObjekt>\n",SiderHeder[i].queryTime.getUserObjekt);
							printf("\t<cmc_conect>%f</cmc_conect>\n",SiderHeder[i].queryTime.cmc_conect);
						#endif
					printf("\t</TIMES>");
	...
	while ((x<FinalSiderHeder.showabal) && (i < (QueryData.MaxsHits * (nrOfServers + nrOfPiServers)))) {
		...
		*/
			...
			printf("\t<PAGEGENERATETIME>%f</PAGEGENERATETIME>\
/*****************************************************************/
//dagur: !mangler file-6349687
/*****************************************************************/

n",Sider[i].pageGenerateTime);
	...
	#else
	if(!mysql_real_connect(&demo_db, "localhost", "boitho",

/*****************************************************************/
//dagur: mangler 6349689

 "G7J7v5L5Y7", BOITHO_MYSQL_DB, 3306, NULL, 0)){
    		printf(mysql_error(&demo_db));
    		exit(1);
	}
	#endif
