%{
/*
 *	(C) SearchDaimon 2004-2008, Written by Magnus Galåen
 *
 *	juni 2008: La til full stÃ¸tte for utf-8-bokstaver.
 *	mai 2007: La til støtte for OR. Syntax: Skriv | foran ord eller frase som skal OR-es. OR og NOT samtidig fungerer ikke.
 *
 *
 *	TODO: Legge til støtte for fraseseparatorer, â la [.,@]
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/*
	...
	Dersom avsluttende parentes mangler, tolker vi det som det skulle vært en etter siste ord i query-et.
*/
...
({letter}|{u8a}|{u8b}|{u8c}|{u8d}|{u8e}|{u8f})({letter}|{u8a}|{u8b}|{u8c}|{u8d}|{u8e}|{u8f}|{infix})({letter}|{u8a}|{u8b}|{u8c}|{u8d}|{u8e}|{u8f})+		{
			    if (yyget_extra(yyscanner)->or_is_on)
				{
				    _qp_word_init('|', yyscanner);
				    yyget_extra(yyscanner)->or_is_on = 0;
...
<COMMAND>({letter}|{u8a}|{u8b}|{u8c}|{u8d}|{u8e}|{u8f})+	{ _qp_word_add( yytext, yyscanner ); _qp_word_exit( yyscanner ); BEGIN INITIAL; }
<CMD_PHRASE>({letter}|{u8a}|{u8b}|{u8c}|{u8d}|{u8e}|{u8f})+	{ _qp_word_add( yytext, yyscanner ); }
<CMD_PHRASE>\"		{ _qp_word_exit( yyscanner );			BEGIN INITIAL; }
<PHRASE>({letter}|{u8a}|{u8b}|{u8c}|{u8d}|{u8e}|{u8f})({letter}|{u8a}|{u8b}|{u8c}|{u8d}|{u8e}|{u8f})+	{ _qp_word_add( yytext, yyscanner ); }
...
static inline void _qp_word_add( char *str, yyscan_t scanner )
{
    ...
    for (i=0,j=0; str[i]!='\0' && j<1023; i++)
	{
		...
		text[j++] = '\'';
	    // (Spesialtilfelle) UnngÃ¥ splittet utf8-kode pÃ¥ slutten:
	    else if (j>1019 && (unsigned char)str[i] >= 0xc0)
		break;
	    else
